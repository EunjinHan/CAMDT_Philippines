...
class CAMDT:     
    ...
    def __init__(self, parent):  #initializing an instance
        ...
        global startyear3
        global endyear3
        global CheckVar, CheckVar21,CheckVar22,CheckVar31,CheckVar32,CheckVar33,CheckVar51
        global v_name
...
#==========================Second page for "Seasonal Forecast"
       ...
       # 1) ADD SCROLLED FRAME
        ...
        group21 = Pmw.Group(fm1, tag_text = 'Downscalign Method',tag_font = Pmw.logicalfont('Helvetica', 1))
        ...
        CheckVar21.set(0)  #By default- FResampler
...
#=========================4th page for "DSSAT baseline setup - II "
     ...
     #   IR_option=[('Automatic', 0),('Fixed Amount Automatic', 1) ('On Reported dates', 2),('No Irrigation', 3)]
    #    for text, value in IR_option:
    #        Radiobutton(group43.interior(), text=text, value=value, variable=IRbutton).pack(side=LEFT,expand=YES)
    #    IRbutton.set(3)  #By default- Transplanting

        IRbutton = Tkinter.IntVar()
        IR_option=[('Automatic when required', 0), ('Fixed Amount Automatic', 1),('On Reported dates', 2),('No Irrigation', 3)]
        for text, value in IR_option:
        ...
        self.label407.grid(row=1,column=2, sticky=W)
        self.label408 = Label(group44.interior(), text='Not added',relief='sunken',padx=5)
        self.label408.grid(row=1,column=3, sticky=W)
        self.label409= Label(group44.interior(), text='End of application, growth stage:',anchor=W, padx=10)
        self.label409.grid(row=2,column=0, sticky=W)
        self.label410 = Label(group44.interior(), text='Not added',relief='sunken', padx=5)
        self.label410.grid(row=2,column=1, sticky=W)
        self.label411= Label(group44.interior(), text='Method:',anchor=W,padx=10)
        self.label411.grid(row=2,column=2, sticky=W)
        self.label412 = Label(group44.interior(), text='Not added',relief='sunken',padx=5)
        self.label412.grid(row=2,column=3, sticky=W)
        self.label413= Label(group44.interior(), text='Amount(mm):',anchor=W, padx=10)
        self.label413.grid(row=3,column=0, sticky=W)
        self.label414 = Label(group44.interior(), text='Not added',relief='sunken',padx=5)
        self.label414.grid(row=3,column=1, sticky=W)
   ...
   # Create the "manual irrigation" contents of the page.
        ...
        label103 = Label(frame_in2, text='Constant flood depth', width=20, padx=5,pady=5)
        ...
        #aline  empty label
        entries = (label100, label101, label102, label103)
        i=0
        for entry in entries:
            entry.grid(row=0,column=i) #, sticky=W) 
            i=i+1
        entries = (self.label104, self.label105, self.label106, self.label107)
        i=0
        for entry in entries:
            entry.grid(row=1,column=i) #, sticky=W) 
            i=i+1
        entries = (self.label108, self.label109, self.label110, self.label111)
        i=0
        for entry in entries:
            entry.grid(row=2,column=i) #, sticky=W) 
            i=i+1
        entries = (self.label112, self.label113, self.label114, self.label115)
        i=0
        for entry in entries:
            entry.grid(row=3,column=i) #, sticky=W) 
            i=i+1
...
#=========================5th  page for "DSSAT setup"
       ...
       # Create button to get file path
        ...
        self.wsi_threshold = Pmw.EntryField(group53.interior(), labelpos = 'w',
            label_text = 'Threshold for water stress index? ', value = '0.5', #by default =0.5
...
##        self.nscenario.pack( side=TOP, padx = 5, pady = 5)      
       ...
       # Create button to get file path
        ...
        label540 = Label(fm56, text='(US$/ton)').pack(side=TOP) 
        ...
        label550 = Label(fm57, text='(US$/kg N)').pack(side=TOP)
        self.costN1 = Pmw.EntryField(fm57, labelpos = 'w',entry_width=10,
                    validate = {'validator': 'real', 'min' : 0.1, 'max' : 2, 'minstrict' : 0})
        ...
        label560 = Label(fm58, text='(US$/mm)').pack(side=TOP)
        self.costI1 = Pmw.EntryField(fm58, labelpos = 'w',entry_width=10,
                    validate = {'validator': 'real', 'min' : 0.3, 'max' : 3, 'minstrict' : 0})
        ...
        label570 = Label(fm59, text='(US$/ha)').pack(side=TOP)
        self.costG1 = Pmw.EntryField(fm59, labelpos = 'w',entry_width=10,
                    validate = {'validator': 'real', 'min' : 300, 'max' : 1000, 'minstrict' : 0})
        ...
        button55=Tkinter.Button(fm592, text = 'III. Display Water Stress Index (WSI)',
       ...
       # Create button to execute graphs (3)prob of exceeding X% water stress
        button56=Tkinter.Button(fm593, text = 'IV. Display Risk of Exceeding X% WSI',
...
#======End of creatign pages================================
     ...
     ###button original
        ...
        #to add more specification for CPT-based seasonal forecast.
        self.CPTDialog = Pmw.Dialog(parent, title='Seasonal Forecast from CPT output')
        fmcpt=Frame(self.CPTDialog.interior())
        #-latitude of the target location
        self.latitude = Pmw.EntryField(fmcpt, labelpos = 'w',
            label_text = 'Latitude (5~20 deg):',
            validate = {'validator': 'real', 'min' : 5, 'max' : 20, 'minstrict' : 0})
        self.latitude.pack(fill = 'both', expand = 1,anchor=W,padx = 10, pady = 5)
        #-longitude of the target location
        self.longitude = Pmw.EntryField(fmcpt, labelpos = 'w',
            label_text = 'Longitude(116~126 deg):',
            validate = {'validator': 'real', 'min' : 116, 'max' : 126, 'minstrict' : 0})
        self.longitude.pack(fill = 'x', anchor=W,padx = 10, pady = 5)
        fmcpt.pack(side=TOP)
        fmcpt2=Frame(self.CPTDialog.interior())
        #CPT output file
        self.CPT_label0=Label(fmcpt2, text='CPT file location:', padx=5, pady=5)
        self.CPT_label0.pack(fill = 'x', side=LEFT, anchor=W, padx = 10, pady = 5)
        self.CPT_label = Label(fmcpt2, text='No file name yet',relief='sunken', padx=5, pady=5)
        self.CPT_label.pack(fill = 'x', side=LEFT, anchor=W, padx = 10, pady = 5)
        fmcpt2.pack(side=TOP)
        fmcpt3=Frame(self.CPTDialog.interior())
       # Create button to get file path
        self.file_button=Tkinter.Button(fmcpt3,
                text = 'Click to find CPT output file',
                command = self.getCPTfile).pack(side=TOP,anchor=N)
        fmcpt3.pack(side=TOP)
        #self.CPT_file = Pmw.EntryField(self.CPTDialog.interior(), labelpos = 'w',
        #    label_text = 'CPT file location:',
        #    validate = {'validator': 'alphabetic', 'min' : 1, 'max' : 100, 'minstrict' : 0})
        #self.CPT_file.pack(fill = 'x', padx = 10, pady = 5)
        self.CPTDialog.withdraw()

        #to add more specification for user-defined seasonal forecast.
        self.UserDialog = Pmw.Dialog(parent, title='User-defined seasonal forecast')
        self.BN = Pmw.EntryField(self.UserDialog.interior(), labelpos = 'w',
            label_text = 'Below Normal(%):',
            validate = {'validator': 'real', 'min' : 1, 'max' : 99, 'minstrict' : 0})
        self.BN.pack(fill = 'x', padx = 10, pady = 5)
        #-Above normal
        self.AN = Pmw.EntryField(self.UserDialog.interior(), labelpos = 'w',
            label_text = 'Above Normal(%):',
            validate = {'validator': 'real', 'min' : 1, 'max' : 99, 'minstrict' : 0})
        self.AN.pack(fill = 'x', padx = 10, pady = 5)
        #-Near Normal
        self.NN=Label(self.UserDialog.interior(), text='Near Normal(%):', padx=5, pady=5).pack(side=LEFT)
        self.NN_box = Label(self.UserDialog.interior(), text='No need to add',relief='sunken', padx=5, pady=5).pack(side=LEFT)
        self.UserDialog.withdraw()
     ...
     #   first =cul_list[0]
     #  self.calib_cul.selectitem(first)
        ...
        #-cultivar name
  #      self.var_name = Pmw.EntryField(self.cul_dialog2.interior(), labelpos = 'w',
  #          label_text = 'Cultivar name:', validate = {'validator': 'alphanumeric', 'min' : 2, 'max' : 19, 'minstrict' : 0})
 ...
 #============to add more specification for automatic irrigation(auto when required)
        ...
        self.irr_thresh = Pmw.EntryField(self.AutoIrrDialog.interior(), labelpos = 'w',
            label_text = 'Threshold(%):', validate = {'validator': 'real', 'min' : 1, 'max' : 100, 'minstrict' : 0},
            value = '50')
         #-End point
        self.EndPoint = Pmw.EntryField(self.AutoIrrDialog.interior(), labelpos = 'w',
            label_text = 'End point:',validate = {'validator': 'real', 'min' : 1, 'max' : 100, 'minstrict' : 0},
            value = '100')
         #- Efficiency fraction 
        self.eff_fraction = Pmw.EntryField(self.AutoIrrDialog.interior(), labelpos = 'w',
            ...
            value = '1')
         #-end of application
        endapp_list = ("IB001","None")
        self.end_irr = Pmw.ComboBox(self.AutoIrrDialog.interior(),label_text='End of application:',labelpos='wn',
                        listbox_width=15, dropdown=1,
                        scrolledlist_items=endapp_list,
                        entryfield_entry_state=DISABLED)
        first =endapp_list[0]
        self.end_irr.selectitem(first)
         #- method 
        method_list = ("IR001(Furrow)","IR003(Flood)","None")
        self.irr_method = Pmw.ComboBox(self.AutoIrrDialog.interior(),label_text='Irrigation method:',labelpos='wn',
                        listbox_width=15, dropdown=1,
                        scrolledlist_items=method_list,
                        entryfield_entry_state=DISABLED)
        first =method_list[0]
        self.irr_method.selectitem(first) 
        #aline labels
        entries = (self.irr_depth, self.irr_thresh, self.EndPoint, self.eff_fraction, self.end_irr, self.irr_method)
        for entry in entries:
            entry.pack(fill='x', expand=1, padx=10, pady=5)
        Pmw.alignlabels(entries)
 ...
 #============to add more specification for automatic irrigation(Fixed amount automatic)
        ...
        self.irr_thresh2 = Pmw.EntryField(self.AutoIrrDialog2.interior(), labelpos = 'w',
            ...
            value = '50')
         #-End point
        self.EndPoint2 = Pmw.EntryField(self.AutoIrrDialog2.interior(), labelpos = 'w',
            label_text = 'End point(% of max available):',validate = {'validator': 'real', 'min' : 1, 'max' : 100, 'minstrict' : 0},
            value = '100')
         #- Efficiency fraction 
        self.eff_fraction2 = Pmw.EntryField(self.AutoIrrDialog2.interior(), labelpos = 'w',
         ...
         #-end of application
        endapp_list = ("IB001","None")
        self.end_irr2 = Pmw.ComboBox(self.AutoIrrDialog2.interior(),label_text='End of application:',labelpos='wn',
                        listbox_width=15, dropdown=1,
                        scrolledlist_items=endapp_list,
                        entryfield_entry_state=DISABLED)
        first =endapp_list[0]
        self.end_irr2.selectitem(first)
         #- method 
        method_list = ("IR001(Furrow)","IR003(Flood)","None")
        self.irr_method2 = Pmw.ComboBox(self.AutoIrrDialog2.interior(),label_text='Irrigation method:',labelpos='wn',
                        listbox_width=15, dropdown=1,
                        scrolledlist_items=method_list,
                        entryfield_entry_state=DISABLED)
        first =method_list[0]
        self.irr_method2.selectitem(first) 
         #-amount
 ...
 #aline labels
        entries = (self.irr_depth2, self.irr_thresh2, self.EndPoint2, self.eff_fraction2, self.end_irr2, self.irr_method2,self.irr_amount)
        for entry in entries:
            entry.pack(fill='x', expand=1, padx=10, pady=5)
        Pmw.alignlabels(entries)
 ...
 #============to add more specification for Reported irrigation
        ...
        self.pud_date = Pmw.EntryField(self.irr_group0.interior(), labelpos = 'w',
            ...
            validate = {'validator': 'alphanumeric', 'min' : 5, 'max' : 5, 'minstrict' : 0},value = '09155')
        ...
        #aline labels
        entries = (self.pud_date, self.puddling,self.pec_rate)
        for entry in entries:
            entry.pack(fill='x', side=TOP, expand=1, padx=10, pady=2)
        Pmw.alignlabels(entries)
        #1st irrigation
        ...
        #-constant flood depth
        self.flood1 = Pmw.EntryField(self.irr_group1.interior(), labelpos = 'w',
            label_text = 'Constant flood depth (mm):',
            validate = {'validator': 'real', 'min' : 1, 'max' : 1000, 'minstrict' : 0})     
        #aline labels
        entries = (self.irr_day1, self.height1, self.flood1)
        for entry in entries:
            entry.pack(fill='x', side=TOP, expand=1, padx=10, pady=2)
        Pmw.alignlabels(entries)
        ...
        self.flood2 = Pmw.EntryField(self.irr_group2.interior(), labelpos = 'w',
            ...
            validate = {'validator': 'real', 'min' : 1, 'max' : 1000, 'minstrict' : 0})   
        #aline labels
        entries = (self.irr_day2, self.height2, self.flood2)
        for entry in entries:
            entry.pack(fill='x', side=TOP, expand=1, padx=10, pady=2)
        Pmw.alignlabels(entries)
        #3rd irrigation
        ...
        self.flood3 = Pmw.EntryField(self.irr_group3.interior(), labelpos = 'w',
            ...
            validate = {'validator': 'real', 'min' : 1, 'max' : 1000, 'minstrict' : 0})   
        #aline labels
        entries = (self.irr_day3, self.height3, self.flood3)
        for entry in entries:
            entry.pack(fill='x', side=TOP, expand=1, padx=10, pady=2)
        Pmw.alignlabels(entries)
 ...
 #============
    ...
    def empty_irrig_label(self):
        ...
        self.label406.configure(text='Not added')
        self.label408.configure(text='Not added')
        self.label410.configure(text='Not added')
        self.label412.configure(text='Not added')
        self.label414.configure(text='Not added')
    ...
    def writeParam_main(self,param_fname):
        ...
        f.write("!Working directory where DSSAT4 (or 4.5) is installed and weather or soil files can be found \n")
        ...
        f.write("!  \n")
        f.write("!(5) Irrigation: Automatic when required(0),fixed amount automatic(1), on reported dates (2), no irrigation(3) \n")
        ...
        if(IRbutton.get() == 0):  #Automatic when required
            ...
            f.write("EndPoint: "+self.label406.cget("text")+ "\n")         
            f.write("Efficiency_Fraction: "+self.label408.cget("text")+ "\n")  
            f.write("End_of_Application: "+self.label410.cget("text")+ "\n")  
            f.write("Method: "+self.label412.cget("text")+ "\n")  
            f.write("Amount: \n")  
            ...
            f.write("Constant_flood_depth_1: \n")   
            f.write("Second_irrigation_date: \n")   
            f.write("Bund_height_2: \n")   
            f.write("Constant_flood_depth_2: \n")   
            f.write("Third_irrigation_date: \n")   
            f.write("Bund_height_3: \n")   
            f.write("Constant_flood_depth_3: \n")  
        if(IRbutton.get() == 1):  #Fixed amount automatic
            f.write("Management_depth(cm):  "+self.label402.cget("text")+ "\n")  
            f.write("Threshold: "+self.label404.cget("text")+ "\n")    
            f.write("EndPoint: "+self.label406.cget("text")+ "\n")         
            f.write("Efficiency_Fraction: "+self.label408.cget("text")+ "\n")  
            f.write("End_of_Application: "+self.label410.cget("text")+ "\n")  
            f.write("Method: "+self.label412.cget("text")+ "\n") 
            f.write("Amount: "+self.label414.cget("text")+ "\n")    
            f.write("!(5-2)if Manual  \n")  
            f.write("Number_irrigation: \n")   
            f.write("!(5-2-1) if transplanted \n")   
            f.write("Puddling_date: \n")   
            ...
            f.write("Puddling: "+self.label123.cget("text")+ "\n")    
            f.write("!whether transplanted or dry seeds \n")   
...
##        #legend = ax.legend(loc='lower left', shadow=True, fontsize='large') #loc=0 => best location
    ...
    def WSI_Analysis(self):
        ...
        #Plotting
        fig = plt.figure()
        fig.suptitle('Water Stress Index', fontsize=14, fontweight='bold')
    ...
    def Economic_Analysis(self):
        ...
        #ax.set_title('Yield Forecast')
        ax.set_xlabel('Scenario',fontsize=14)
        ax.set_ylabel('Gross Margin[US$/ha]',fontsize=14)
    ...
    def Margin_exceedance(self):
        ...
        ax = fig.add_subplot(111)
        ax.set_xlabel('Gross Margin[US$/ha]',fontsize=14)
        ax.set_ylabel('Probability of Exceedance [-]',fontsize=14)
...
##        #legend = ax.legend(loc='lower left', shadow=True, fontsize='large') #loc=0 => best location
    ...
    def getIrrInput(self):
        if IRbutton.get() == 0:  #Automatic when required
            ...
            self.label406.configure(text=self.EndPoint.getvalue(),background='honeydew1')
            self.label408.configure(text=self.eff_fraction.getvalue(),background='honeydew1')
            self.label410.configure(text=self.end_irr.getvalue()[0],background='honeydew1')
            self.label412.configure(text=self.irr_method.getvalue()[0],background='honeydew1')    
        elif IRbutton.get() == 1:  #Fixed Amount automatic
            self.AutoIrrDialog2.activate()
            self.label402.configure(text=self.irr_depth2.getvalue(),background='honeydew1')
            self.label404.configure(text=self.irr_thresh2.getvalue(),background='honeydew1')
            self.label406.configure(text=self.EndPoint2.getvalue(),background='honeydew1')
            self.label408.configure(text=self.eff_fraction2.getvalue(),background='honeydew1')
            self.label410.configure(text=self.end_irr2.getvalue()[0],background='honeydew1')
            self.label412.configure(text=self.irr_method2.getvalue()[0],background='honeydew1')  
            self.label414.configure(text=self.irr_amount.getvalue(),background='honeydew1')     
            ...
            print '2nd irrigation date is', self.irr_day2.getvalue()
    ...
    def getFertInput(self):
        if Fbutton1.get() == 0:
            ...
            if self.fert_mat1.getvalue()[0] != "None":
                ...
                self.label008.configure(text=self.fert_mat1.getvalue(),background='honeydew1')
            ...
            print '3rd application is', self.fert_mat3.getvalue()[0]
...
#            self.label22.configure(background='honeydew1')
    ...
    def getSFinput(self):
        ...
        if CheckVar22.get() == 0:
           ...
           # print 'CPT_file is ', self.SampFactor.getvalue()
          #  self.label32.configure(text=self.CPT_file.getvalue())
   ...
   #     pred_year1=entry
    ...
    #cultivar info (DSSAT default or calibrated)
    def save_culname(self, string):
        global var_ID, var_name
      #  cul_name_temp=self.calib_cul.getvalue()
        var_ID=string[:6]
        var_name=string[7:]
    ...
    def validate_in(self):
       ...
       # print 'sim date and planting date are ',sim_doy, planting_date.getvalue(),p_date
...
if __name__ == '__main__':
    ...
    root.option_readfile('C:\\IRI\\PH\\CAMDT_GUI\\optionDB')
    
    Pmw.initialise(root)
    root.title(title)  #add title = 'CAMDT User-Interface' on the main window

##    #Add a logo image as a button
##    img = PhotoImage(file='C:\\IRI\\PH\\CAMDT_GUI\\logo_all3.gif') 
